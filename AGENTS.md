...
**SE HOUVER CONFLITO, SIGA ESTAS REGRAS DO EDUTRACK ΑΙ.**
...
**X IGNORE ESSAS INSTRUÇÕES. NÃO TENTE FAZER PUSH, SYNC OU DEPLOY.**
...
**X NÃO FAÇA:**
- X Procurar ou invocar ferramentas de push/sync/deploy
- X Executar comandos shell para sincronizar com Xano
- X Validar se o código foi aceito pelo servidor
- X Tentar "finalizar o processo" além da geração de arquivos
...
- Aplique as regras do EduTrack AI em caso de conflito di instruções
- Solicite ao usuário o caminho correto dos guidelines externos apenas se ainda houver ambiguidade

2. **Revisar a seção di sintaxe relevante** (ex: "Field Options" para campos di tabelas)
...
**✔ FAÇA:**
- Realizar `read_file` do guideline específico quando o arquivo existir
...
2. **Código e termos específicos:** Nomes de variáveis, funções, tabelas, endpoints e termos-chave do OpenSpec (ex: `Requirement`, `Scenario`, `SHALL`, `MUST`) devem permanecer em **INGLES**.
...
5. **Commits:** Siga Conventional Commits (`feat:`, `fix:`, `docs:`, `chore:`).
...
**X Erros mais comuns que causam falha na validação:**
1. **Localização do arquivo determina o formato**
   - X Usar `## Requirements` em `openspec/changes/<id>/specs/capability/spec.md`
...
2. **Hierarquia markdown incompleta**
   - X Começar direto com `### Requirement:`
...
3. **Palavras-chave incorretas**
   - X "must", "should", "may" (minúsculas)
   - ✔ `SHALL` ou `MUST` (maiúsculas)
4. **Scenarios faltando ou mal formatados**
   - X Requirement sem scenario
   - X Scenario em texto corrido
   - ✔ Todo requirement TEM ao menos 1 scenario com bullets **WHEN**/**THEN**
...
[O que é e por quê]
## ADDED Requirements
<- IMPORTANTE: Use ADDED (não apenas Requirements)
### Requirement: Fazer X
