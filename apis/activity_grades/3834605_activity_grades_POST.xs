...
query "activity_grades" verb=POST {
  ...
  stack {
    ...
    db.add activity_grades {
      ...
      output = ["id", "created_at", "activity_id", "student_id", "grade", "teacher_id", "comment"]
    } as $new_grade
    ...
    function.run "Getting Started Template/create_event_log" {
      input = {
        ...
        metadata  : {grade_id: $new_grade.id, activity_id: $new_grade.activity_id, student_id: $new_grade.student_id, grade: $new_grade.grade}
      }
    } as $event_log
  }
